class EbayApiClient
  ...
  def delete_payment_policy(policy_id)
    delete("/sell/account/v1/payment_policy/#{policy_id}")
  rescue => e
    Rails.logger.error "Unexpected error deleting payment policy: #{e.message}"
    nil
  end
